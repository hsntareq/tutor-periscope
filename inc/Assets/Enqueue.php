...
class Enqueue {
	...
	public function inline_script_data(): array {
		...
		if ( 'tutor_quiz' === $post_type ) {
			...
			$evaluation_form_id   = (int) Form::course_has_form( $tutor_course_id );
			$ef_form_has_question = ( new FormField() )->get_all_fields_by_form_id( $evaluation_form_id );
			...
			if ( $last_attempt ) {
				...
				if ( $earned_percentage >= $passing_grade && ! $already_evaluated && count( $ef_form_has_question ) ) {
